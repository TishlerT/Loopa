...
- **TrackMixerRow.swift**: Single track row for Tracks mixer. Compact single-row layout with delete button, track icon, name, M/S/Q/L buttons (horizontal), horizontal volume slider (inline after buttons), and instrument picker. All controls on one line for maximum track visibility. Includes QuantizeOptionsSheet and InstrumentPickerSheet.
